...
def list_hours():
    ...
    if auth.has_membership('air'):
        ...
        if form.accepted:
            response.flash = T("Hours added, wait for approval")
    if auth.has_permission('approve', 'hours'):
        name = lambda row: row.student.first_name + ' ' + row.student.last_name
        apr = lambda row: [A(T('Change'), _href=URL('toggle_hour', args=row.id)), name(row)]
...
def manage_hour():
    ...
    form = SQLFORM(db.work_hour, hour_id, deletable=True).process()
